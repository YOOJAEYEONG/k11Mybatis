...
public class JSONController {
	...
	public String aList(Model model, HttpServletRequest req, ParameterDTO parameterDTO) {
		...
		*/
		...
		//페이지 번호에 대한 처리 :Ajax로 처리하기위해 메소드 변경
	...
	@ResponseBody	//JSON데이터를 만들어주는 어노테이션
	...
	//글쓰기 처리
	@RequestMapping(method = RequestMethod.POST, value = "/mybatisJSON/writeAction.do")
	...
	public Map<String, Object> deleteAction(HttpServletRequest req, HttpSession session){
		...
		/*
		Mybatis사용시 delete쿼리를 실행하게 되면 처리후 실제 삭제된 행의 갯수를 반환한다.
		이를 통해 컨트롤러에서는 성공/실패 여부를 판단할 수 있다.
		*/
		if(result<=0) {
			map.put("statusCode", 0);//삭제실패시
		}else {
			map.put("statusCode", 2);//삭제성공시
		}
		return map;
	}
